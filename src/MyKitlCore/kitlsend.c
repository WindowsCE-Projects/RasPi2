...
static void SendStringToSerial (LPWSTR pszStr)
{
    ...
    if (!(KITLDebugZone & KITL_ZONE_NOSERIAL)) {
        AcquireKitlSpinLock ();        
        KITLOutputDebugString ("%d PzD:%x TID:%x ",
    ...
    DWORD dwUserDataLen )   // @parm [IN] - Data length (up to KITL_MAX_DATA_SIZE).
{
    ...
    if ((KITL_SVC_DBGMSG == Id) && pCurTh && (pCurTh->bDbgCnt > 1)) {
        ...
        if (SEQ_DELTA(pClient->AckExpected, pClient->TxSeqNum) >= pClient->WindowSize) {
            ...
            //KITL_DEBUGMSG(1, (L"KITLSend(%u): Tx window full (AckExpected: %u, Seq: %u) ********\r\n",
//                                        Id,pClient->AckExpected,pClient->TxSeqNum));
        }
		else
#endif
		{
			...
			//KITL_DEBUGMSG(1, (L"+KITLSend(%u), Len: %u, Seq: %u, AckExpected: %u  %d\r\n",
//								 Id, dwUserDataLen,pClient->TxSeqNum, pClient->AckExpected, OEMGetTickCount()));
		}
        while (fRet && SEQ_DELTA(pClient->AckExpected, pClient->TxSeqNum) >= pClient->WindowSize) {

			KITLOutputDebugString("+KITLSend(%u), Waiting for free spot in queue***********\n", Id);
        ...
        if (fRet) {
            ...
            if (!fRet) {
				...
				//SendStringToSerial(L"!KITLSend: **** (db) Inserting Pkt in timer\r\n");
				...
				//SendStringToSerial(L"KITLSend: Waiting for Ack\r\n");
                // polling mode, poll ACK
                PollAck (pClient, PollBuf);
				//SendStringToSerial(L"KITLSend: Ack recvd\r\n");
	...
	//KITL_DEBUGMSG(1, (L"-Send (%u): Ret %u\r\n", Id, fRet));
...
BOOL DoKITLSendInSysCall (UCHAR Id, UCHAR *pUserData, DWORD dwUserDataLen)
{
	...
	//KITL_DEBUGMSG(1, (L"DoKITLSendInSysCall++ id:%d\r\n", Id));
    ...
    if (KitlSendFrame (pFrame, pClient->TxFrameLen[dwPacketIndex])) {
        // poll ACK
		SendStringToSerial(L"DoKITLSendInSysCall: Waiting for Ack\r\n");
        PollAck (pClient, PollRecvBuf);
		SendStringToSerial(L"DoKITLSendInSysCall: Ack recvd\r\n");
	...
	//KITL_DEBUGMSG(1, (L"DoKITLSendInSysCall-- id:%d err:%d\r\n", Id, dwErr));
    ...
    DWORD dwUserDataLen )   // @parm [IN] - Data length (up to KITL_MAX_DATA_SIZE).
{
	...
	//KITL_DEBUGMSG(1,(L"KITLSend++ id:%d\r\n", Id));
	...
	//KITL_DEBUGMSG(1, (L"KITLSend-- id:%d rc:%d\r\n", Id, fRet));
    return fRet;
}

void KITLWriteDebugString (LPCWSTR str)
{
	//KITL_DEBUGMSG(1, (L"KITLWriteDebugString++\r\n"));
