...
BOOL HandleRecvInterrupt (LPBYTE pRecvBuf)
{
	...
	//KITL_DEBUGMSG(1, ("HandleRecvInterrupt++ buff:%x\n"),pRecvBuf);
    for(;;) {
        pHdr = KitlRecvFrame(pRecvBuf, &wMsgLen);
        if(pHdr == NULL) {
            break;
        }
		//KITL_DEBUGMSG(1, ("HandleRecvInterrupt: buff:%x id:%d cmd:%d\n"), pRecvBuf, pHdr->Id, pHdr->Cmd);
	...
	//KITL_DEBUGMSG(1, ("HandleRecvInterrupt-- buff:%x retran:%d\n"),pRecvBuf, fRetransmit);
    ...
    DWORD       Timeout)   // @parm [IN] - Timeout value (0 == don't block, INFINITE == wait forever)
{
    ...
    KITL_DEBUGMSG(ZONE_RECV,("+KITLRecv(%u): Waiting for message (expect Seq %u). User bufsize: %u timeout %d\n",
                             pClient->ServiceId, pClient->RxSeqNum, *pdwLen, Timeout));
    ...
    while (!pClient->RxFrameLen[pClient->NextRxIndex]) {
        ...
        KITL_DEBUGMSG (1,("Z Waiting...\n"));
            if (WAIT_TIMEOUT == DoWaitForObjects (1, &pClient->phdRecvEvt, Timeout)) {
        KITL_DEBUGMSG (1,("Y Timeout!\n"));
                // interrupt mode, block on receive event
                dwErr = ERROR_TIMEOUT;
                break;
            }
        KITL_DEBUGMSG (1,("X  Signaled\n"));
