        ...
        if (Kitl.pfnEncode (SendFmtBuf, cbData)) {
            ...
            do {
                ...
                do {
					...
					KITL_RETAILMSG(ZONE_INIT, (("KITLConnectToDesktop: s:%x != %x  s:%x != %x\n"), pHdr->Service, KITL_SVC_ADMIN, pHdr->Cmd, KITL_CMD_TRAN_CONFIG));

                    // only care about admin transport config packet
                    if ((KITL_SVC_ADMIN == pHdr->Service) && (KITL_CMD_TRAN_CONFIG == pHdr->Cmd)) 
					{
					...
					{
		                KITL_RETAILMSG(ZONE_INIT, (("KITLConnectToDesktop: Received non-admin, non-config packet %x  %x\n"), pHdr->Service, pHdr->Cmd));

						// (db) Added this
		                if ((KITL_SVC_ADMIN == pHdr->Service) && (KITL_CMD_RETRASMIT == pHdr->Cmd)) 
						{
		                    KITL_RETAILMSG(ZONE_INIT, (" KITLConnectToDesktop: Received retransmit packet. retransmitting\n"));
							goto retransmit_frame;
						}
                    }
skip_process_frame:
                    if (fUseTick) {
