...
int MailboxSendMail (int nMailbox, int nChannel, DWORD dwData)
{
	...
	PDWORD pdwMBStat = (PDWORD)((DWORD)pdwMBBase + MAILBOX_STATUS_OFF);
	PDWORD pWriteMB = (PDWORD)((DWORD)pdwMBBase + MAILBOX_WRITE_OFF);

	//Debug (L"pdwMBBase:%x  pdwMBStat:%x	 pWriteMB:%x\r\n", pdwMBBase, pdwMBStat, pWriteMB );
...
int MailboxGetMail (int nMailbox, int nChannel, DWORD *pdwData)
{
	...
	PDWORD pdwMBStat = (PDWORD)((DWORD)pdwMBBase + MAILBOX_STATUS_OFF);
	PDWORD pReadMB = (PDWORD)((DWORD)pdwMBBase + MAILBOX_READ_OFF);
