...
static BOOL SerialRecv(UINT8 *pData, UINT16 *pLength)
{
	...
	KITL_RETAILMSG(ZONE_RECV, ("+SerialRecv(0x%x, %d) pl:%d\r\n", pData, *pLength, recvPayload));
    ...
    while (recvCount < (sizeof(OAL_KITL_SERIAL_HEADER) + recvPayload)) {
        ...
        // Only read the requested amount of data since the remainder gets thrown away
        toRead = sizeof(OAL_KITL_SERIAL_HEADER) + recvPayload - recvCount;
        if (toRead > bestSize) toRead = bestSize;

        // Call driver read function, with best size
        count = g_kitlSerialState.pDriver->pfnRecv(&pBuffer[recvCount], toRead);

        // Break loop when no are avaiable
        if (count == 0) 
		{

            if (g_kitlSerialState.pDriver->pfnFlowControl && (dwRetry ++ < RECV_RETRY)) 
			{
                continue;
            }
            break;
...
BOOL OALKitlSerialInit(
    ...
    KITL_RETAILMSG (ZONE_KITL_OAL, ("+OALKitlSerialInit('%s', '%S', 0x%x, 0x%x)\r\n",
    ...
    {
        KITLOutputDebugString("WARN: KITL will run in polling mode\r\n");
    }
    else
    {
        KITLOutputDebugString("KITL will run in interrupt mode\r\n");
    }
