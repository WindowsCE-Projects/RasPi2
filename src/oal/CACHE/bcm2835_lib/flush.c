...
{
	//OALLogSerial (TEXT("OEMCacheRangeFlush++ pAddress %x, length %d, flags %x\r\n"), pAddress, length, flags);
	...
	//OALLogSerial (TEXT("OEMCacheRangeFlush--\r\n"));
}
...
OALLogSerial (TEXT("1.2\r\n"));

    if ((flags & CACHE_SYNC_FLUSH_I_TLB) != 0) {
OALLogSerial (TEXT("6\r\n"));
        if (length == PAGE_SIZE) {
OALLogSerial (TEXT("6.1\r\n"));
            // flush one TLB entry
            OALClearITLBEntry(pAddress);
OALLogSerial (TEXT("6.2\r\n"));
        } else {
OALLogSerial (TEXT("6.3\r\n"));
            // flush the whole TLB
            OALClearITLB();
OALLogSerial (TEXT("6.4\r\n"));
        }
    }
OALLogSerial (TEXT("1.3\r\n"));
