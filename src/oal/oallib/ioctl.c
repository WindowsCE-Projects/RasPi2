...
BOOL OALIoCtlHalGetDisplayConfig(UINT32 code, VOID *pInpBuffer, UINT32 inpSize, 
								 VOID *pOutBuffer, UINT32 outSize, UINT32 *pOutSize);
...
//------------------------------------------------------------------------------
//
//  Function:  OALIoCtlHalGetDisplayConfig
//
//  Implements the IOCTL_HAL_MY_GET_DISPLAY_CONFIG handler.  This is called by
// the video driver to query the display settings.
								 ...
								 VOID *pOutBuffer, UINT32 outSize, UINT32 *pOutSize)
{
	...
	NKDbgPrintfW(L"OALIoCtlHalGetDisplayConfig++\r\n");

    UNREFERENCED_PARAMETER(code);    
    UNREFERENCED_PARAMETER(pOutBuffer);    
    UNREFERENCED_PARAMETER(outSize);    

	if (pInpBuffer && (inpSize == sizeof (MYDISPLAYSETTTNGS)))
	{
		__try 
		{
			// This routine is located in debug.c
			fRC = GetVideoSettings ((PMYDISPLAYSETTTNGS)pInpBuffer);
			if (fRC && pOutSize)
				*pOutSize = sizeof (MYDISPLAYSETTTNGS);
		}
		__except (EXCEPTION_EXECUTE_HANDLER)
		{
//			SetLastError(ERROR_INVALID_PARAMETER);
	...
	NKDbgPrintfW(L"OALIoCtlHalGetDisplayConfig-- rc %d\r\n", fRC);
