...
#define BURST_1    3
		...
		volatile DWORD Reserved3:12;      // 08-20 
...
#define GRXSTSREG_PAKSTAT_SETUP_UPDT	0x06 // Setup packet
	...
	typedef struct {
		...
		volatile DWORD GAhbCfgReg;   //0x08  Global AHB Config reg
		...
		volatile DWORD Reserved1;    //0x30  
		volatile DWORD Reserved2;    //GPvndCtlReg;  //0x34  Global  reg
		...
		DWORD Reserved4[0x74];
		volatile HPTXFSIZREG HPTxfSizReg;  //0x100  Global Periodic Xmit Fifo reg
	...
	// 0x400 Host Config
	typedef struct {
#define HOSTCLK_HSFSLS     0
#define HOSTCLK_LIMIT2FSLS 1
		...
		volatile DWORD Reserved3:1;    // 23 (Scatter/gather only)
		volatile DWORD Reserved4:2;    // 24-25 (Scatter/gather only)
		volatile DWORD Reserved5:1;    // 26 (Scatter/gather only)
	...
	//------------------------------------
	// 0x404 Frame Number/Frame Time remaining
	typedef struct {
	...
	typedef struct {
		...
		volatile DWORD PrtSpd:1;       // 17 Port speed 0-HS, 1-FS, 2-LS
...
#define CHAN_CHAR_TYPE_INTR   3
	...
	typedef struct {
		...
		volatile DWORD HubAddr:7;      // 06-13 Hub Address
...
#define DWCPID_MDATA           3   //High Speed (480mbps)
